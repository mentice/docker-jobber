...
A neural network is like a black box - we don't know how it works, but we can adjust its inputs and observe its outputs.
...
Docker images are stored in Docker registries using an efficient layered approach that reduces both total storage size as well as transfer times when pushing or pulling images.
...
After the job completes (either successfully or unsuccessfully), a snapshot of the filesystem is saved to yet another image (by default using a tag with '`-latest-run`' appended).
...
Additional [TODO](meta data) is written linking the result image to the code and data images from which it was created.
...
Configuration names are listed on the command line using the `-c` [option](#GlobalOptions).
...
Values from the setting dictionary are merged with the current state of settings maintained during the directory search.
Encountering a configuration name causes its value to also be merged as if it had occurred on the command line.
This results in a surprisingly flexible and easy to use system for defining different development and runtime environments.
...
The code may modified, debugged, and executed multiple times during development.
